...
def result():
    ...
    canvas._tkcanvas.place(width=225, height=225, x=600, y=240)
    ...
    canvas2._tkcanvas.place(width=225, height=225, x=860, y=240)
    ...
    canvas3._tkcanvas.place(width=225, height=225, x=600, y=480)
    ...
    canvas4._tkcanvas.place(width=225, height=225, x=860, y=480)


    ####main logic of the prediction model
    ##input data
    ...
    if(prediction == 1):
        ...
        print("The person doesn't have a heart disease hence he is having a healthy heart")
        report.config(text=f"Report:{0}", fg="#8dc63f")
        report1.config(text=f"{name}, you don't have a heart disease")
...
##functionality in logout button
def logout():
    root.destroy()
...
#icon - 1
image_icon = PhotoImage(file="images/heartbeat.png")
root.iconphoto(False, image_icon)


#header section - 2
logo = PhotoImage(file="images/doctor.png")
...
Label(Heading_entry, text="Registration No.", font="arial 12", bg="#df2d4b", fg=framefg).place(x=30, y=0)
Label(Heading_entry, text="Date", font="arial 12", bg="#df2d4b", fg=framefg).place(x=430, y=0)

Label(Heading_entry, text="Patient Name", font="arial 12", bg="#df2d4b", fg=framefg).place(x=30, y=90)
Label(Heading_entry, text="Birth Year", font="arial 12", bg="#df2d4b", fg=framefg).place(x=430, y=90)
...
Registration=IntVar()
reg_entry=Entry(Heading_entry, textvariable=Registration, width=20, font="arial 20", bg="#ededed", fg="#222222", bd=0)
reg_entry.place(x=30,y=45)

Date=StringVar()
today=date.today()
d1=today.strftime("%d/%m/%Y")
date_entry=Entry(Heading_entry, textvariable=Date, width=15, font="arial 20", bg="#ededed", fg="#222222", bd=0)
...
square_report_image=PhotoImage(file="images/phone.png")
report_background=Label(image=square_report_image, bg=background)
report_background.place(x=1000, y=275)

report=Label(root, text="hello", font="arial 25 bold", bg="white", fg="#8dc63f")
report.place(x=1190, y=540)

report1=Label(root, text="hello", font="arial 10 bold", bg="white")
...
Label(image=graph_image).place(x=600, y=250)
Label(image=graph_image).place(x=860, y=250)
Label(image=graph_image).place(x=600, y=490)
Label(image=graph_image).place(x=860, y=490)


#result button - 10

analysis_button=PhotoImage(file="images/result.png")
Button(root, image=analysis_button, bd=0, bg=background, cursor="hand2", command=result).place(x=1165, y=210)


#info - 11

info_button=PhotoImage(file="images/info.png")
Button(root, image=info_button, bd=0, bg=background, cursor='hand2', command=Info).place(x=40, y=350)


#save - 12
save_button=PhotoImage(file="images/save.png")
Button(root, image=save_button, bd=0, bg=background, cursor='hand2').place(x=1370, y=225)


#smoking and non-smoking button - 13
button_mode=True
choice="smoking"
...
smoking_icon=PhotoImage(file="images/green_smoking.png")
non_smoking_icon=PhotoImage(file="images/red_smoking.png")

mode=Button(root, image=smoking_icon, bg="#dbe0e3", bd=0, cursor="hand2", command=changemode)
mode.place(x=350, y=488)
...
logout_button.place(x=1300, y=60)
